...
- **路径严禁硬编码（动态路径机制）**：
  ...
  - **临时文件原子安全发布**：生成过程写入 `<输出目录>/.tmp_<进程ID>_<文件名>.xlsx`，并通过 `<输出目录>/.lock_<文件名>.xlsx` 防止同一报表并发生成；写入完毕后以原子替换发布目标文件。
...
2. **排序规则**：家电电脑审核失败记录按关联得到的 `商品名称` 升序排列，商品名称为空的记录排在最后；数码区块没有商品名称列，保持源数据顺序。
3. **相对行号生成**：
...
2. **排序规则**：家电电脑与数码两个区块分别按 `交易参考号` 升序排列，空值排在最后。
3. **相对行号生成**：
...
- **`finance_wb.rs`**：工作簿二（4 个 Sheet）的银联双键索引、状态级联判定与 60 列跨类对齐合并。
...
# 源数据不在工程同级 source_data/ 时，显式指定目录
SUBSIDY_SOURCE_DIR=/path/to/source_data cargo test --test acceptance_test --release -- --nocapture
```
...
| **审核失败明细** | 失败记录笔数及排序 | 206 笔（家电电脑 132 笔 + 数码 74 笔）；家电电脑按商品名称升序、空值置后 | 严格一致 |
| **异常回款明细** | 本店异常回款笔数及排序 | 46 笔（家电电脑 44 笔 + 数码 2 笔）；两个区块分别按交易参考号升序、空值置后 | 严格一致 |
...
| **3.上传明细**  | 数据行数与列数 | 12,010 行数据（家电电脑 4,825 + 数码 7,185），60 列 | 严格一致 |
