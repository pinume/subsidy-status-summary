...
pub fn generate_store_finance_workbook(input_dir: &Path, output_path: &Path) -> Result<(), String> {
    ...
    // Extract store merchant codes (with uniqueness validation)
...
fn build_final_match_sheet(
    ...
    let all_ref_idx = all_h
        .find(&["交易参考号"])
        .ok_or("银联所有缺少交易参考号")?;
    let all_ord1_idx = all_h
        .find(&["银商订单号"])
        .ok_or("银联所有缺少银商订单号")?;
    ...
    for (idx, row) in all_union.iter().enumerate().skip(1) {
        let ref_no = cell_to_string(&row[all_ref_idx]);
        if !ref_no.is_empty() {
            ref_map.entry(ref_no).or_insert(idx);
        }

        let ord1 = cell_to_string(&row[all_ord1_idx]);
        if !ord1.is_empty() {
            order_map.entry(ord1).or_insert(idx);
        }

        if let Some(idx2) = all_ord2_idx {
            let ord2 = cell_to_string(&row[idx2]);
            if !ord2.is_empty() {
                order_map.entry(ord2).or_insert(idx);
            }
        }
    }
    ...
    for (r_idx, row) in store_occ.iter().enumerate().skip(1) {
        ...
        let prod_row = ref_map
            .get(&ref_num)
            .or_else(|| order_map.get(&ord_num))
            .map(|&idx| &all_union[idx]);
        let (cat, brand, model) = match prod_row {
            Some(r) => (
                cell_to_string(&r[all_cat_idx]),
                cell_to_string(&r[all_brand_idx]),
                cell_to_string(&r[all_model_idx]),
            ),
