...
pub fn generate_summary_workbook(input_dir: &Path, output_path: &Path) -> Result<(), String> {
    ...
    // Extract store merchant codes from uploaded files (with uniqueness validation)
    let app_store_code = extract_unique_merchant_code(&app_upload_rows, "已上传家电电脑.xlsx")?;
    let dig_store_code = extract_unique_merchant_code(&dig_upload_rows, "已上传数码.xlsx")?;
    ...
    build_invoice_anomaly_sheet(
        &mut workbook,
        &styles,
        &input_dir.join("发票明细.xlsx"),
        &invoice_rows,
    )?;
...
fn build_summary_sheet(
    ...
    let t2_data: [(&str, Decimal, f64, Decimal, f64, Decimal, f64); 8] = [
        (
            "国补发生额及占比",
            app_gen_amt,
            div_pct(app_gen_amt, tot_gen_amt),
            dig_gen_amt,
            div_pct(dig_gen_amt, tot_gen_amt),
            tot_gen_amt,
            1.0,
        ),
        (
            "国补回款额及回款率",
            app_paid_amt,
            div_pct(app_paid_amt, app_gen_amt),
            dig_paid_amt,
            div_pct(dig_paid_amt, dig_gen_amt),
            tot_paid_amt,
            div_pct(tot_paid_amt, tot_gen_amt),
        ),
        (
            "回款+审核通过未回款及综合回款率",
            app_paid_amt + app_pass_amt,
            div_pct(app_paid_amt + app_pass_amt, app_gen_amt),
            dig_paid_amt + dig_pass_amt,
            div_pct(dig_paid_amt + dig_pass_amt, dig_gen_amt),
        ...
        (
            "未回款额",
            app_unpaid_amt,
            div_pct(app_unpaid_amt, app_gen_amt),
            dig_unpaid_amt,
            div_pct(dig_unpaid_amt, dig_gen_amt),
            tot_unpaid_amt,
        ...
        (
            "审核通过未回款",
            app_pass_amt,
            div_pct(app_pass_amt, app_unpaid_amt),
            dig_pass_amt,
            div_pct(dig_pass_amt, dig_unpaid_amt),
            tot_pass_amt,
        ...
        (
            "待审核",
            app_wait_amt,
            div_pct(app_wait_amt, app_unpaid_amt),
            dig_wait_amt,
            div_pct(dig_wait_amt, dig_unpaid_amt),
            tot_wait_amt,
        ...
        (
            "审核失败",
            app_fail_amt,
            div_pct(app_fail_amt, app_unpaid_amt),
            dig_fail_amt,
            div_pct(dig_fail_amt, dig_unpaid_amt),
            tot_fail_amt,
        ...
        (
            "未上传",
            app_unup_amt,
            div_pct(app_unup_amt, app_unpaid_amt),
            dig_unup_amt,
            div_pct(dig_unup_amt, dig_unpaid_amt),
            tot_unup_amt,
